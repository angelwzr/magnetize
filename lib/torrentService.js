...
async function validateEgress(hostname) {
    try {
        ...
        if (ip.isPrivate(address) || ip.isLoopback(address)) {
            const error = new Error(`SSRF Blocked: Private IP detected (${address})`);
            error.status = 403;
            throw error;
        }
    } catch (err) {
        if (err.status === 403) throw err;
        const error = new Error(`Invalid URL or hostname: ${hostname}`);
        error.status = 400;
        throw error;
    }
}
...
async function _fetchFromUrl(url) {
    ...
    try {
        ...
        const error = new Error(`Failed to fetch torrent data from URL: ${err.message}`);
...
async function handleTorrentSource(source, isMagnet = false, options = {}) {
    ...
    if (!input || input.length === 0) {
        const error = new Error('No torrent data provided');
        error.status = 400;
        throw error;
    }
    ...
    let parsed;
    try {
        parsed = parseTorrent(input);
    } catch (err) {
        const error = new Error(`Failed to parse torrent data: ${err.message}`);
        error.status = 400;
        throw error;
    }
